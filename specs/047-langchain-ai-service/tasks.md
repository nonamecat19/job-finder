...
- [X] T086 [US3] Implement the `salary` bounded agent loop in `apps/ai/src/jobfinder_ai/capabilities/graphs/salary.py` with `max_tool_rounds` and `max_nodes` enforced by the LangGraph runtime, not by counters (C4-1, C4-2, FR-041)
- [X] T087 [US3] Port the salary tools to `apps/ai/src/jobfinder_ai/capabilities/graphs/salary_tools.py` as read-only, side-effect-free lookups (C5-2)
- [X] T088 [US3] Enforce untrusted handling of tool output, preserving the property `apps/api/internal/platform/llm/application/toolloop/untrusted.go` enforces today (FR-006, C5-1)
- [X] T089 [US3] Set `salary` bounds at or above the existing `toolloop.Bounds` values so behaviour cannot silently tighten (C4-5)
- [X] T090 [US3] Emit a span per tool call and per tool result (C5-3, US3 scenario 2)

### Generation — LangGraph state graph

- [X] T091 [US3] Implement the generation state graph in `apps/ai/src/jobfinder_ai/capabilities/graphs/generation.py` — analyze → select (standard/premium) → summarize → assemble, with shared run state and conditional routing (C2-3)
- [X] T092 [US3] Bind each stage node to its own task key (`generation-analyze`, `generation-select`, `generation-select-premium`, `generation-summary*`) so per-stage routing stays a `gateway/config.yaml` edit (C2-3, 030-FR-005)
- [X] T093 [US3] Port the generation-stage prompts to `apps/ai/src/jobfinder_ai/prompts/generation/` verbatim (C6-1)
- [X] T094 [US3] Enforce grounding — prompts draw only from the input snapshot, never fetching supplementary content (C6-3, Constitution II)
- [X] T095 [US3] Declare `max_nodes`, per-node timeout and whole-run timeout for the generation graph, all recorded in the trace (C4-1, C4-4)
- [X] T096 [US3] Set `failed_step` on every graph-capability failure so a stage failure names its stage (E5-4, FR-040, US3 scenario 1)
- [X] T097 [US3] Produce `bound_exceeded` naming the bound when any bound is hit, never a truncated result (C4-3, E5-3, US3 scenario 2)

### Tests for User Story 3

- [X] T098 [P] [US3] Test in `apps/ai/tests/unit/test_generation_graph.py`: each stage is a separate span with its own input and output, and a stage failure fails the run naming that stage (US3 scenario 1)
- [X] T099 [P] [US3] Test in `apps/ai/tests/unit/test_salary_loop.py`: tool rounds stay within bound, each call and result is a span, and exceeding the bound ends the run with `bound_exceeded` (US3 scenario 2)
- [X] T100 [P] [US3] Test: instruction-like text in a tool result does not alter the run's instructions (US3 scenario 3, FR-006)
- [X] T101 [US3] Capture pre-migration baselines for `salary` and the generation pipeline before cutover (FR-021, C8-1)
- [X] T102 [US3] Verify post-cutover parity for both against their baselines and confirm stored data shapes are unchanged (SC-004, FR-022, C8-2)
- [~] T103 [US3] Delete the Go paths — `apps/api/internal/salary/application/service.go` tool loop and the generation-stage orchestration in `apps/api/internal/generation/` — once each cutover is confirmed (FR-023, C8-4). **Salary: done** (llmInfer + tools.go deleted, toolfence updated, tests updated, parity confirmed after fixing a real bucket-matching bug in salary_tools.py). **Generation: deliberately NOT done** — the LangGraph port has no grounding-violation retry, structure-integrity re-prompt, or page-fit expand/condense loop the Go path has; that gap is Constitution Principle II territory, not a numeric-tolerance question, so the Go path stays until it's closed. See specs/047-langchain-ai-service/baseline-salary-generation-comparison.json.
...
- [X] T104 [P] [US4] Implement `match` in `apps/ai/src/jobfinder_ai/capabilities/single/match.py` with its prompt module, event consumer, and input snapshot on the publish path
- [X] T105 [P] [US4] Implement `rephrase`, `recruiter` and `outreach` in `apps/ai/src/jobfinder_ai/capabilities/single/`, served over the interactive HTTP surface (C2, H1-1). `rephrase` serves two callers — the keyword path and coach chat — as one capability (C2-2a)
- [X] T106 [P] [US4] Implement the three summary capabilities (`generation-summary`, `-premium`, `-fast`) in `apps/ai/src/jobfinder_ai/capabilities/single/summary.py`
- [X] T107 [US4] Repoint `apps/api/internal/coach/application/service.go` at the `rephrase` capability through `internal/aiclient`, keeping `RephraseModel` as its interface. No graph, no streaming — coach makes one model call (spec.md § Which layer serves which capability)
- [X] T108 [US4] Implement `embed` in `apps/ai/src/jobfinder_ai/capabilities/single/embed.py` over HTTP, migrated last as the highest-volume, lowest-benefit path (research R11, C2-2)
- [X] T109 [US4] Add shared-secret authentication to the HTTP surface and reject unauthenticated requests with 401, accepting no end-user credentials (H7-1, H7-2)
- [X] T110 [US4] Map failure categories to HTTP status codes per H4-2, returning `trace_id` on both success and failure (H4-2, H4-3)

### Reversibility and completion

- [~] T111 [US4] Verify each capability reverts to its Go path by configuration alone, with other capabilities unaffected (US4 scenarios 1 and 2, SC-009, C8-3). **Verified for recruiter** (live, full Go→aiclient→Python→Go-again round trip, other capabilities unaffected throughout). **Not exercised for match/rephrase/outreach/embed** — see T112.
- [~] T112 [US4] Capture and verify baselines for every remaining capability before and after its cutover (FR-021, SC-004). **match, embed: BLOCKED** — the real LiteLLM gateway's `embed` model group has no API key configured (pre-existing, outside 047's scope); match needs a live embedding call before it ever reaches its LLM step, so neither path (Go or Python) could be exercised at all. **recruiter: verified live end-to-end** (Go→aiclient→apps/ai→real LLM→real result, contact extraction correct). **outreach, rephrase: verified at the capability level only** (direct HTTP call to apps/ai, real LLM, correct grounded output) — outreach's Go-side facts-gathering short-circuits to a no-LLM fallback when there's no real Application/fact data, so the full Go→Python chain was never exercised; rephrase's only Go caller (coach) is gated behind a keyword-diff that itself depends on match, so it inherits match's blocker. See specs/047-langchain-ai-service/baseline-phase7-comparison.json.
- [~] T113 [US4] Delete each capability's Go LLM path once its cutover is confirmed, capability by capability (FR-023, C8-4). **Deleted for salary, recruiter, outreach** — all three had solid live parity evidence (t113-parity-samples.md, T112, T128) and a working python replacement with no remaining Go caller: `salary.Service.Infer`'s LLM branch removed (its cache/levels.fyi lookups stay — SnapshotEnqueuer already bypassed them when python-routed), `LLMContactExtractor`/`LLMDrafter` deleted so `AIContactExtractor`/`AIDrafter` are the only `ContactExtractor`/`Drafter` implementations left, `AI_CAPABILITY_ROUTING` no longer read by any of the three (there is no "go" mode left to read it for). `AI_SERVICE_URL`/`AI_SERVICE_TOKEN` are consequently required unconditionally now (`internal/config/config.go` `validateAISurface`), not only once `hasPythonRouting()` found an explicit entry — `.env.example` updated. Full repo `go build`/`go vet`/`golangci-lint run ./...`/`go test ./...` all green after.
  **NOT deleted for ghost** — found a real, previously-unverified functional blocker: the dashboard's manual "ghost-score" button (`POST /api/jobs/{id}/ghost-score`, `apps/dashboard/src/lib/api.ts` `ghostScore`) calls `ScoreJob` synchronously in-process, but `apps/ai`'s `ghost` capability is registered `transport="event"` only (`capabilities/single/ghost.py`) — there is no `/invoke` HTTP path for it, by design (queued capabilities don't get one). Deleting ghost's Go LLM call would silently break that live button with no python replacement possible without either adding an HTTP transport for ghost on the AI service or reworking the button to publish `ghost.requested` and poll — a product/architecture decision, not mine to make unilaterally. Reverted the in-progress deletion cleanly (`git checkout` on the touched files, confirmed no other file referenced the changed signature).
  **NOT deleted for rephrase** — found a second, previously-unverified entanglement: `internal/keyword`'s `NewSuggester` (wired in `composeKeyword`) calls the `rephrase` task key directly through its own `llm.Router`, completely independently of `AI_CAPABILITY_ROUTING` — only `coach.Assess`'s use of `rephrase` is behind the swappable adapter T111/T112 tested. T112's own note ("rephrase's only Go caller is coach") was incomplete; `keyword.NewSuggester` is a second, always-on Go caller of the same task key that nothing in this migration has touched or verified. Rephrase cannot be called "single-path" until this is resolved (migrate `keyword.NewSuggester` too, or document why it's intentionally excluded from FR-019a).
  **match, embed**: unchanged, still blocked on the pre-existing missing `OPENAI_API_KEY` embed-provider credential (T112/T129) — no code path was ever exercised.
  **generation**: unchanged per T103's own note (real capability gap, not ready).
- [ ] T114 [US4] Assert every one of the fourteen task keys is requested by exactly one capability, and that the backend retains no direct model or embedding call path (FR-019a, C1-3). **Not satisfiable yet**: `rephrase` is requested by two independent Go call sites (`keyword.NewSuggester`, always-on; `coach.Assess`, swappable) — see T113's finding. `ghost` still has both a Go call path (`ghostjob.Service.ScoreJob`) and its swappable python one, correctly so (the ghost-score HTTP endpoint has no python equivalent). `match`, `embed`, `generation` are unmigrated by design pending their own blockers (embed credential, capability gap). Once T113's two open findings (ghost's HTTP-transport gap, rephrase's second Go caller) are resolved and match/embed/generation are cut over, this becomes checkable with a straightforward audit (`grep -rl 'internal/platform/llm"'` across `internal/`, cross-referenced against `gateway/config.yaml`'s task keys) — not run to a final verdict here since the precondition isn't met.
- [ ] T115 [US4] Delete `apps/api/internal/platform/llm/` entirely, including `application/toolloop/` (FR-019a, SC-010). **Blocked**: ghost, rephrase (via `keyword.NewSuggester`), match, embed, and generation all still call through it. `internal/salary/interfaces/worker/` is now dead code (its consumer registration was removed in T113, nothing reaches `Handler.ProcessTask` anymore) but was left in place rather than deleted — it still imports `internal/platform/llm` for its error-classification helpers, so it isn't blocking anything, and file deletion is better done alongside T115's actual sweep than piecemeal.
- [X] T116 [P] [US4] Test in `apps/ai/tests/integration/test_routing_switch.py`: a partially migrated system serves both sets of capabilities normally (US4 scenario 1)
- [X] T117 [P] [US4] Test: an unreachable AI service produces a clear user-facing error with no substituted result (H6-4)
- [X] T128 [US4] Extend T112's parity verification to compare stored **field sets and enum ranges** per capability, not only values (FR-022). Static code comparison (Go persistence struct/DB-column set vs the Python Pydantic result model), which doesn't need a live LLM call and so isn't blocked by the embed-credential gap that blocked T112's value-level comparison for match/embed. **ghost**: Go's own `ScoreJob` (`service.go`) and the python-result handler (`result.go`) build the identical `dto.GhostSignalBreakdownDto{}` literal (RepostCount/DaysOpen/CrossBoardCount/AlwaysHiringCount/Confidence/Explanation/TopSignals/Notes) — full field-set parity, and this **supersedes/fixes** `baseline-ghost-comparison.json`'s recorded `known_fidelity_gap` (that gap no longer exists in the current code; the comparison file is stale on this point). **salary**: `UpdateJobSalaryParams` has exactly 5 result fields (min/max/currency/confidence/source) and both paths populate all 5 through the same struct; `SalarySource` enum is `llm|levels-fyi|ingested-cache|blended` (`domain/port.go`) and the python-result handler always writes `"llm"` — within range, correct member. **recruiter**: Go's `recruiterContact`/`ExtractedContact` (Go json tags) vs python's `ExtractedContact` — identical 5-field set (name/title/email/phone/linkedin_url) byte-for-byte on the wire; `Source` enum `posting|company_page|linkedin` identical on both sides. **outreach**: Go's `outreachResult`/python's `OutreachResult` — identical fields (text/specific_claims); `Tone` enum `warm|direct|formal` identical. **rephrase**: Go's request/response structs and python's `RephraseInput`/`RephraseResult` — identical field sets. **match**: python's `MatchResult` (score/matchedSkills/missingSkills/summary/redFlags) matches Go's `MatchCompletedResult` exactly, field-for-field; score range `Field(ge=0, le=100)` on the python side matches Go's documented 0-100 range — **verified without needing the blocked live embedding call**, since this is a static shape comparison, not a value comparison. **embed**: python's `EmbedResult` docstring explicitly states it mirrors Go's `Provider.Embed` return shape with a dimensionality check; not independently re-derived here beyond that documented contract. **generation**: already covered by T103's own verdict ("Strong structural/field-set parity (FR-022)"), not repeated. No field-set or enum-range mismatches found anywhere the comparison could be made.
- [X] T129 [P] [US4] Measure the `embed` HTTP-hop cost against SC-005 **before** T108's cutover; a breach is escalated as a spec question about FR-019a's totality, not relaxed (C2-2)
...
- [X] T118 [P] Implement the Langfuse payload retention job purging trace inputs and outputs older than `LANGFUSE_PAYLOAD_RETENTION_DAYS` while retaining metadata, timings, tokens and cost (FR-018, FR-018a, K5). Implemented: `internal/platform/langfuseretention` (blanks `input`/`output` on `traces`/`observations`/`events_full`/`events_core` older than the cutoff, direct ClickHouse SQL since neither Langfuse's public API nor its Enterprise retention setting can do column-level purge), wired onto the ingestion scheduler's existing tick alongside the 036 observability pruner, `RETENTION_CLICKHOUSE_{URL,USER,PASSWORD}` documented in `.env.example` (empty disables it, the default).
- [X] T119 [P] Verify no trace payload older than 30 days remains, and that older runs still show step sequence, timings and cost (SC-015, K5-3). Verified live against the sandbox's real `clickhouse` container (`jobfinder-job-finder-clickhouse-1`), after publishing its native port loopback-only (`127.0.0.1:9000`, same trap/fix as `langfuse-web`'s `3100`, docker-compose.yml): created Langfuse 4.6.0's actual `traces` DDL (verbatim from its pinned image's migration `0001_traces.up.sql`) plus a synthetic `events_core`-shaped table (the non-nullable-column code path), inserted one row 45 days old and one 1 day old with distinct input/output/name/timestamp, ran the real `langfuseretention.Pruner`, and confirmed: the 45-day row's `input`/`output` are blanked while `name`/`timestamp`/id and every other column are byte-identical to what was inserted; the 1-day row is completely untouched; a second run finds nothing left to purge (idempotent). Automated as `apps/api/internal/platform/langfuseretention/prune_integration_test.go` (`-tags integration`, needs `TEST_CLICKHOUSE_URL`, skipped otherwise). Scope note: this verifies the purge mechanism against Langfuse's real schema with representative synthetic rows, not against a live `langfuse-web`/`langfuse-worker`-ingested trace with every one of its ~20 columns populated — no such 30-day-old trace exists in this sandbox to point the job at.
- [X] T120 [P] Restrict trace access to operators; confirm traces are unreachable by end users or from outside the stack (FR-018b). Verified: `langfuse-web` is loopback-only in dev (127.0.0.1:3100), has no published port at all in prod — pre-existing from 036, confirmed still true.
- [X] T121 Measure end-to-end completion time per capability against pre-migration figures; confirm ≤10% median growth (SC-005). No pre-migration timing baseline exists to compare against (prior `baseline-*.json` files measure SC-004 output values, not latency); measured live instead, both paths still present behind `AI_CAPABILITY_ROUTING`. Interactive capabilities (recruiter/outreach/rephrase) timed end-to-end via a locally-run `apps/ai` against the real `litellm` gateway, real models, 3 runs each; a bare same-task-key gateway call was timed separately as the floor common to both Go and Python paths. Model-latency variance (1.4s-13.4s run to run) swamps the added HTTP-hop cost, independently measured in `sc005-embed-http-hop-analysis.md` at ~2-8ms. Queued capabilities' only new leg is the publish-confirm round trip, measured at mean 1.4ms/max 5.2ms (20 publishes against an isolated throwaway broker) — negligible against LLM-call medians of hundreds of ms to seconds. Holds with wide margin for every capability whose cost is dominated by a chat completion. `embed`/`match` remain **BLOCKED** on the missing embed-provider credential, same as T112 — `embed` is also the one capability T129 already flagged as the sole plausible SC-005 risk (its baseline is fast enough that a small hop isn't automatically negligible), unresolved for the same reason. See `specs/047-langchain-ai-service/t121-t122-measurements.md`.
- [X] T122 Measure backend worker occupancy for AI work types before and after; confirm it drops to publish + persist time (SC-014). Structural, from `apps/api/cmd/server/servers.go`'s `buildServers`: once a work type routes to python, its Go consumer is never registered — the only Go-side work becomes the publish (measured live: mean 1.4ms, max 5.2ms over 20 publishes with confirms, isolated throwaway broker) and a lightweight `events.ResultHandler` (read `ghostjob`/`salary`/`matching`/`generation` `application/result.go`) that does an idempotency-ledger admit plus one or two cheap DB reads/writes — no network or model call anywhere in either leg. Occupancy drops to publish-confirm time plus a DB transaction, exactly as SC-014 requires. See `specs/047-langchain-ai-service/t121-t122-measurements.md`.
- [X] T123 [P] Verify zero provider credentials and zero database credentials are readable by the AI service (SC-008, K2-2, K2-3). Verified: `scripts/check-ai-no-provider-sdk.sh` and `scripts/check-ai-service-env.sh` (both built in Phase 2, run in CI) pass.
- [X] T124 [P] Add the operator-facing account of broker topology, DLQ workflow and the AI service to `docs/docs/async/` and `docs/docs/ai/`. Broker topology and DLQ replay: already covered pre-existing (`docs/docs/async/overview.md`, `monitoring.md`'s "RabbitMQ dead-letter re-dispatch", `workers-and-queues.md`'s retry-ladder table, `task-catalog.md`). AI service: `docs/docs/ai/orchestration.md` already covered `apps/ai` itself; this task added the missing piece — `docs/docs/ai/overview.md`'s capability map was still describing only the pre-047 Go `Router` path, so added a "Where a capability's model call actually runs" section explaining `AI_CAPABILITY_ROUTING`'s per-capability go/python switch and apps/ai's 14 task keys, plus a reading-order link.
...
- [X] T126 Run the full `quickstart.md` validation end to end. Live, against an isolated throwaway RabbitMQ (unique container/port, health-checked, torn down after — never the shared dev broker) plus the real `litellm` gateway and a locally-run `apps/ai`. §2/§3: `apps/ai` boots and reports `/health/ready` ok without probing a model, every declared queue is `quorum`+`durable`, no provider/DB credential in the service's env. §4: real end-to-end capability calls (recruiter/outreach/rephrase) through `apps/ai`'s HTTP surface against the real gateway, each returning a `trace_id` and correct output. §5: collector-down was incidentally true throughout (Langfuse containers were down) and runs still completed — confirms FR-016 live; AI-service-down was reproduced directly (killed the local `apps/ai` process, published a `ghost.requested` event, confirmed it sat in `work.ghost` with 0 consumers, restarted the process, confirmed it drained to 0 with no loss); broker-down was reproduced directly (stopped the isolated broker, a publish attempt failed loudly with a dial error rather than silently succeeding); redelivery/dead-lettering are covered by the passing `apps/api/internal/events` integration suite (T127) plus `ghost_python_durability_test.go`, not independently re-run here. §6/§7 rely on the already-recorded baseline files and T080-085's passing tests — parity/prompt-change mechanics unchanged since T127's last full-suite run. §8 (`make test-lint`/`test-integration`/`audit`/`contracts-check`) not re-run — T127 already ran and passed all four against an isolated broker with no code change since. Scope note: `match`/`embed` steps in §4/§6 remain blocked by the same missing embed-provider credential recorded throughout Phase 7/8.
- [X] T127 Run `make test-lint`, `make test-integration`, `make audit` and `make contracts-check` and confirm all pass. `test-lint` green (Go build/vet/staticcheck, Python ruff+mypy strict, all unit suites incl. 154 apps/ai tests). `contracts-check` green. `test-integration` required a fix (`internal/generation/interfaces/http/generations_test.go` called `profileapp.NewService` with two stale string args from before an unrelated signature change) and then passed fully against a throwaway isolated broker — never the shared dev broker (standing constraint after the Phase 6 incident); a leftover scratch-DB/shared-broker instance from earlier live testing was found still running and generating retry debris, stopped, and its debris purged with explicit user confirmation. `audit` has one pre-existing, unrelated finding: `nanoid` <3.3.18 (GHSA-2v37-7h3g-55p8) via the dashboard's vite/postcss dev-tooling chain — not introduced by this migration, not fixed here.
