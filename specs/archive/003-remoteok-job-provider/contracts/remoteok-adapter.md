...
- `Kind()` → `dto.SourceKindAPI` (see research.md#R1 (`research.md#r1-fetch-mechanism--json-api-vs-html-scrape`, removed on merge — see git history)).
- `Search`:
  ...
  - **Postcondition (success)**: returns `[]dto.NormalizedJob` with `SourceKey: "remoteok"`
    ...
    research.md#R3 (`research.md#r3-response-shape-and-the-leading-legal-notice-element`, removed on merge — see git history)).
  - **Pacing**: a single call issues one HTTP request in the common case; no pagination
    ...
    research.md#R7 (`research.md#r7-rate-limiting--request-pacing`, removed on merge — see git history)).
...
- **Postcondition (success)**: `Available: true`, `Description` populated from the
  ...
  research.md#R5 (`research.md#r5-fetchdetail--enrichment-shape`, removed on merge — see git history)).
