...
- `Kind()` → `dto.SourceKindScrape` (see research.md#R3 (`research.md#r3-fetch-mechanism--html-scrape-vs-api`, removed on merge — see git history)).
- `Search`:
  ...
  - **Precondition (credentials)**: if neither `JOBLEADS_EMAIL` nor `JOBLEADS_PASSWORD` is
    ...
    research.md#R1 (`research.md#r1-access-model--login-gated-vs-public`, removed on merge — see git history)).
  ...
  - **Postcondition (session expired)**: if the login-page redirect is detected after one
    ...
    research.md#R4 (`research.md#r4-login-page--session-expiry-detection`, removed on merge — see git history)), returns a
  ...
  - **Pacing**: paginated fetches MUST wait at least 500ms between requests and MUST NOT
    ...
    research.md#R8 (`research.md#r8-rate-limiting--request-pacing`, removed on merge — see git history)).
