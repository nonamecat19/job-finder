...
- `Kind()` → `dto.SourceKindScrape` (see research.md#R2 (`research.md#r2-fetch-mechanism--html-scrape-vs-api`, removed on merge — see git history)).
- `Search`:
  ...
  - **Postcondition (blocked/throttled)**: if the response looks like a rate-limit or
    ...
    research.md#R3 (`research.md#r3-blockedthrottled-response-handling`, removed on merge — see git history)), page 1 returns a
