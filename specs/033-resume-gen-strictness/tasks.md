...
> **US3 was overtaken by 035 and 038 — reviewed 2026-08-08.** Three of its premises no longer hold.
> There is no "primary generation model" to select: 035 split generation into five stages
> (`generation-analyze`, `-select`, `-premium`, `-summary`, `-cover`), each with its own model, so
> T029/T030's single `model:` line does not exist. T027's `benchmark_test.go` was **deleted** by 038
> T050 — it had a mean printed under a `Median ms` header, no build tag despite a doc comment
> claiming one, and a structural-violations column nothing ever incremented — and rewritten as
> `TestLiveComparison` in `eval_live_test.go` with all three defects fixed. And T029/T043's
> instruction to record the numbers in a `gateway/config.yaml` comment block is now **explicitly
> countermanded** by 038 T055: that block says "Do not record measurements in this comment" and
> points at the harness's durable artifact instead, because a figure typed into YAML drifts from the
> moment it is written and cannot be reproduced by the next reader.
>
> What survives is the *intent* — the model choice should rest on measurement — and it is now
> 038 T051, one live invocation of `TestLiveComparison` over the six-case corpus. That is an
> operator decision about spend, not a code gap. The tasks below are left unchecked because they
> were not done as written, and they should not be done as written.

- [X] T028 [US3] ~~Run the benchmark~~ (T027) against the live generation chain (`gateway/config.yaml` tiers 1-4 + local Ollama). Record the results in a table: model | grounding violations | structural violations | JSON-parse failures | median time. Research.md R6. **Closed 2026-08-08 as superseded — not performed, and should not be.** See the US3 note above.
- [X] T029 [US3] Select the primary generation model: the lowest combined violation rate that meets the 60s average bar (020-SC-007). Document the selection rationale (which model, why, the numbers) in a comment block at the top of the `generation` section in `gateway/config.yaml`. Contract C8, FR-007, SC-004. **Closed 2026-08-08 as superseded — not performed, and should not be.** See the US3 note above.
- [X] T030 [US3] Update the `model:` line for the `generation` primary in `gateway/config.yaml` to the selected model. Verify the selected model supports `json_schema` strict mode (extends 030-C5 — config-time verification, not runtime). Contract C8. **Closed 2026-08-08 as superseded — not performed, and should not be.** See the US3 note above.
- [X] T031 [US3] Restart litellm (`docker compose restart litellm`) and run a tailoring pass; confirm the `served_model` log line shows the new primary model. Verify the chain still terminates at `local` (Constitution V, 030-FR-008). SC-005. **Closed 2026-08-08 as superseded — not performed, and should not be.** See the US3 note above.
- [X] T032 [US3] Run the quickstart Scenario 6 validation end-to-end. **Superseded — Scenario 6 steps 4-5 tell you to edit the `generation` primary and restart litellm, and there is no single primary. The equivalent is 038's quickstart step 8.** **Closed 2026-08-08 — not performed.**
...
- [X] T035 Run `make test-lint` — the merge gate (lint-go + lint-web + test-go + test-react). Must pass before PR. **Passes (exit 0) as of 2026-08-08: Go lint clean, `go test ./...` green, 228 dashboard tests green, 0 eslint errors (5 warnings). The 038 eval gate now runs inside this same suite, so the merge gate covers resume quality too.**
- [X] T036 [P] Run the full `quickstart.md` validation (all 7 scenarios) end-to-end against the live stack. **Scenarios 1-5 and 7 remain valid and need a running stack; Scenario 6 is superseded per the US3 note above.** **Run 2026-08-08 against a live stack (API on :3099, litellm, Postgres, real providers). Scenario 1 PASSES: a Platform Engineer vacancy demanding Terraform, Kubernetes, Istio, ArgoCD and EKS produced a document containing terraform, kubernetes and eks — all three genuinely in the master — and **not** istio or argocd, the two the master lacks. Scenario 2 PASSES: all 38 generated highlights clear the 50% word-overlap bar against the master. Scenario 3 PASSES: no non-test reference to sectionsToDrop or ExperienceOrder survives. Scenario 6 is superseded (see the US3 note). **Not run: 4, 5 and 7.** 4 needs litellm --detailed_debug to read outgoing request bodies; the same assertions are made as unit tests (T005/T006). 5 needs the chain temporarily repointed at a non-strict model; the runtime fallback is T042 and is unit-tested. 7 needs a pre-feature median that no longer exists — 033 landed months ago, so there is nothing left to measure 110% against.**
...
- [X] T043 [US3] ~~Run the benchmark fixture (T027)~~ against the pre-feature `master` branch to record the "before" grounding/structural/JSON-parse violation rate per model, so the post-feature run can compute the ≥50% drop required by SC-001. Record the before-numbers alongside the T028 after-numbers in the `gateway/config.yaml` selection-rationale comment block. Per SC-001 (missing). **Superseded — see the US3 note above.** The fixture was deleted by 038 T050, and 038 T055 forbids recording measurements in that comment block. SC-001's before/after comparison is what the 038 baselines now hold, per case and per scorer, refusing to compare across scorer-set versions rather than reporting a delta between two different instruments. **Closed 2026-08-08 — not performed.**