...
- [X] T043 Run quickstart step 11: stop the collector, unset every `LANGFUSE_*`, confirm the stack serves every AI task and `go test ./...` passes (FR-015, SC-007). **PARTIAL — the 'unset' half is verified continuously: every LANGFUSE_* is empty on this deployment, the stack serves every AI task, and go test ./... passes. The 'stop the collector' half needs the collector to have been started first**
