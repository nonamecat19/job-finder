...
- [X] T027 [US2] Add `lint-go`, `lint-web` and `lint` targets to `/home/nnc/Projects/job-finder/Makefile` per contracts/make-targets.md
- [X] T028 [US2] Redefine `test-lint` in the `Makefile` as `lint-go lint-web test-go test-react`, keeping `test-integration` and `test-e2e` out of it — they need containers and a browser
- [X] T029 [P] [US2] Delete the dead `"test:python": "make test-python"` script from `/home/nnc/Projects/job-finder/package.json` — the make target does not exist, so the script fails on invocation. **This feature owns this line** (feature 024 verifies it but does not edit it)
...
- [X] T031 [US2] Add the `lint (go)` and `lint (web)` jobs to `/home/nnc/Projects/job-finder/.github/workflows/api-ci.yml` using the exact job names from contracts/required-checks.md, and add both to that file's declared gating set (FR-003, FR-014) — names are a contract, since a rename silently drops a check from the set the phase-2 ruleset consumes
- [X] T032 [US2] Verify per quickstart.md P2: `time make lint` green in <60s (SC-004); a planted Go violation and a planted dashboard violation each reported with location (FR-011); `make lint 2>&1 | grep -E "sqlcgen|generated\.ts"` returns nothing (FR-010); `PATH=/usr/bin:/bin make lint-go` fails with an install line rather than passing (FR-027) — all confirmed: `make lint` ran in ~3s, planted violations in `apps/api/internal/config/config.go` and `apps/dashboard/src/lib/api.ts` were each reported with file:line and rule, the generated-path grep returned nothing, and `PATH=/usr/bin:/bin make lint-go` failed with the golangci-lint install line
- [ ] T033 [US2] Verify local and automated verdicts agree (SC-005): with the CI lint jobs live, run `make test-lint` locally on this branch, push, and compare the local result against `gh run view` for the same commit. They must reach the same verdict on style, static analysis and unit tests. A disagreement means a pinned version or a config path differs between the two, which is the failure this feature's version pinning exists to prevent — record the comparison so the ≥95% agreement target has a baseline — **not run**: this requires pushing this branch to GitHub and inspecting a live Actions run, which is outside what a local/offline implementation pass can execute. `make test-lint` is confirmed green locally (see T032); comparing against a live `gh run view` result is left for whoever pushes this branch and opens the PR
...
- [X] T034 [US3] Add the `integration test` job to `/home/nnc/Projects/job-finder/.github/workflows/api-ci.yml` with service containers **`pgvector/pgvector:pg16`** (not stock postgres — fixtures build `vector(768)` values, see `internal/matching/integration_test.go`) and `redis:7-alpine`, setting `POSTGRES_DB: jobfinder_test` directly so no `createdb` step is needed
- [X] T035 [US3] Implement the **two-step** run inside that job, ordering matters: first `go test -tags integration ./internal/db/... -run TestMigrate`, then `go test -tags integration ./...`. Only `internal/db`'s suite calls `Migrate()` (`internal/db/integration_test.go:31`); the other 12 tagged packages assume a schema, and `go test ./...` runs packages in parallel. The advisory lock in `internal/dbtest/lock.go` serialises `TRUNCATE`, not schema creation
- [X] T036 [US3] Set `DATABASE_URL` and `REDIS_URL` for both steps per contracts/required-checks.md, and add **no** `COMPOSE_PROJECT_NAME`/`POSTGRES_HOST_PORT` handling — those exist to stop 12 local worktrees colliding on one host; a CI runner is a fresh container
- [X] T037 [P] [US3] Add the `e2e (playwright)` job with triggers `pull_request`, `push` to master, `schedule: '0 3 * * *'` and `workflow_dispatch` (FR-019). Needs **no** database, Redis or Go backend — `feed.spec.ts` and `sources.spec.ts` mock every call via `page.route`, `navigation.spec.ts` asserts headings only, and `playwright.config.ts` already handles `webServer` and `reuseExistingServer: !process.env.CI`
- [ ] T038 [US3] Confirm FR-020 failure surfacing: a failed scheduled run emails the workflow author by default, which satisfies "without polling" for a solo maintainer — verify it actually arrives rather than assuming — **not run**: requires a live scheduled GitHub Actions run, which needs this branch pushed and merged first
- [ ] T039 [US3] Verify per quickstart.md P3: `make test-integration` and `make test-e2e` green locally; both new jobs green on a pull request; a deliberately broken migration turns `integration test` red; `gh workflow run "API CI"` dispatches — **partially run, found a pre-existing blocker**: `make test-integration` requires a local `.env` with `DB_PASSWORD` (dev-environment prerequisite, not something this feature configures) and could not be exercised end-to-end here. More importantly, `go vet -tags integration ./...` — what the new CI job actually runs — currently fails to *compile*, independent of any infrastructure or of this feature: `internal/jobsources/adapters/atsboard_integration_test.go` references an undefined `ads.DefaultClient()` and an `emptyQuery` struct shape that no longer matches `dto.SearchQuery`. This is pre-existing broken test code (not introduced by this change), out of scope to redesign here (it needs understanding of an HTTP-client-swap mocking strategy that was seemingly never finished), but it means the `integration test` CI job this task adds will be **red on day one** for a reason unrelated to any future change — flagged prominently for the maintainer. `make test-e2e`/Playwright locally: 2 of 9 specs pass; see T041 for the one fix in scope, and the note below for the rest.
- [ ] T040 [US3] Verify re-runnability (FR-021): take a job that failed for an infrastructure reason rather than a code reason, run `gh run rerun --failed`, and confirm it passes **without a new commit**. Every job must be re-runnable against the unmodified change, which holds only while no job depends on mutable external state — so also confirm no job reaches a live third-party job board or any resource outside the run's own service containers — **not run**: requires a live CI run to re-run; by inspection, no job added here calls a live third-party job board or depends on mutable state, so it should hold, but this needs confirming against a real run
- [X] T041 [US3] `navigation.spec.ts` did need real data in CI — confirmed by running the suite locally: without a backend, the app shell's unconditional `notifications/unseen-count` and `profiles/config/status` calls fail with `ECONNREFUSED` and destabilize every route this spec visits. Added a `test.beforeEach` with `page.route` mocks for both endpoints (matching the mocking style already used in feed.spec.ts/sources.spec.ts) rather than booting the API in CI, per plan Risks. This fixes the ECONNREFUSED-driven failures. Running the full suite locally, however, surfaces further **pre-existing** failures unrelated to this feature and out of scope to fix here: `feed.spec.ts` expects a `Search title/company…` placeholder that no longer matches the current input (likely stale since the "infinite-scroll card grid" feed rewrite), and both `navigation.spec.ts` and `sources.spec.ts` hit Playwright strict-mode violations from duplicate text/headings introduced by the HeroUI dashboard rewrite (e.g. both an `<h1>Application tracker</h1>` and a sidebar `<h2>Application Tracker</h2>`, and "adzuna" appearing as both a list label and a button). These are real UI/test drift from other feature work landing after these specs were written, not something this feature's scope covers (Out of Scope: "Changing what the ... end-to-end suites assert"); flagged for a follow-up.
...
- [X] T042 [P] [US4] Write `/home/nnc/Projects/job-finder/scripts/hooks/regen-sqlc.sh`: run `make sqlc-generate`, always exit 0 (`PostToolUse` cannot block), report regenerated files via `hookSpecificOutput.additionalContext`, and fail loudly with an install line if `sqlc` is absent
- [X] T043 [P] [US4] Write `/home/nnc/Projects/job-finder/scripts/hooks/regen-tygo.sh`: same shape, running `make tygo-generate`. Note in a comment that this closes only **half** the drift gap while `packages/shared/src/index.ts` remains a hand-maintained duplicate — feature 024 removes the duplicate, after which this hook suffices alone
- [X] T044 [P] [US4] Write `/home/nnc/Projects/job-finder/scripts/hooks/go-postedit.sh`: read `tool_input.file_path` from stdin, `gofmt -w` that file, `go vet` its package only, always exit 0, report via `additionalContext`. Never repository-wide (FR-029)
- [X] T045 [US4] Write `/home/nnc/Projects/job-finder/scripts/hooks/session-verify.sh`: scope from `git diff --name-only` (merge base plus unstaged) — Go paths touched → `make lint-go test-go`; dashboard/shared paths → `make lint-web test-react`; neither → exit 0 immediately. Exit 2 on failure, which **does** block for `Stop`
- [X] T046 [US4] Add loop safety to `session-verify.sh`: block at most once per `session_id` (from the stdin JSON) using a marker under the system temp dir, so a second consecutive failure reports without blocking and a session can always end (research R8)
- [X] T047 [US4] Register all four bindings in `/home/nnc/Projects/job-finder/.claude/settings.json` with the exact `if` path filters from contracts/hooks.md: `Edit(apps/api/**/*.go)`, `Edit(apps/api/internal/db/queries/*.sql)`, `Edit(apps/api/internal/dto/*.go)`, plus the matcher-less `Stop` entry; set a `timeout` on each, generous enough for the 2-minute session budget
- [X] T048 [US4] Verify no recursion (FR-030): confirm neither `apps/api/internal/db/sqlcgen/**` nor `packages/shared/src/generated.ts` matches any `if` filter, and check it empirically by feeding `regen-sqlc.sh` a `sqlcgen/models.go` path and observing a no-op — confirmed for both regen-sqlc.sh (fed a sqlcgen/models.go path) and regen-tygo.sh (fed a generated.ts path): both silently no-op, exit 0
- [X] T049 [US4] Verify per quickstart.md P4: editing a `.sql` query regenerates `sqlcgen` in the working tree for review; editing a DTO regenerates `generated.ts`; badly formatted Go comes back formatted (`gofmt -l apps/api/internal` silent); a failing test blocks the stop; the second `Stop` call with the same `session_id` exits 0 even while still broken — all confirmed. Editing a DTO's regen actually found and fixed real pre-existing drift in packages/shared/src/generated.ts (see the "fix(shared): regenerate stale tygo output" commit). A deliberately broken Go test caused `session-verify.sh` to exit 2 with the failure in stderr; a second call with the same `session_id` exited 0 (loop safety); removing the broken test restored a clean exit 0 with a fresh `session_id`
- [X] T050 [US4] Verify nothing rewrites files at commit time (FR-026): read `.githooks/pre-commit` and confirm it contains no formatting, regeneration or staging step — it may only *reject*. Regeneration must stay visible in the working tree where the author reviews it before committing; a hook that silently fixed files at commit time would put unreviewed machine edits into every commit, which is why FR-026 forbids it even though it would be convenient — confirmed by reading the file: it only computes the branch and exits 0/1
- [X] T051 [US4] Verify the budget (SC-011): touch only a dashboard file, then time `session-verify.sh` — under 2 minutes, with Go suites skipped entirely — verified the skip logic in an isolated synthetic repo (a branch with only a dashboard-path file touched produces a `git diff --name-only` containing only that path, so `GO_TOUCHED` stays 0 and `make lint-go test-go` is never invoked); could not additionally time this scenario in the feature repo itself because this implementation's own diff against origin/master necessarily touches both apps/api and apps/dashboard, making every real scoping check here exercise both suites (confirmed at ~7s total, still comfortably under budget)
...
- [X] T052 [P] Document the emergency override procedure (FR-005) and the one-hour trunk-recovery expectation (SC-012) in `AGENTS.md`
- [X] T053 [P] Add `make setup-hooks` to the setup instructions in `AGENTS.md` and `README.md` — it must run once per clone, and an unactivated hook is an absent gate
- [X] T054 Re-read `AGENTS.md` end to end against the actual `Makefile` and workflow file, confirming no statement is false (SC-010) — the failure mode this feature exists to remove — found and fixed two remaining imprecisions while re-reading: the DTO-regeneration bullet and the DTO/index.ts convention note both singled out `dto.go`, but DTOs are split across 11 files in `apps/api/internal/dto/*.go` (jobs.go, profiles.go, queue_backlog.go, ...) and the hook's `if` filter matches all of them; broadened both to say so. Also switched the tygo bullet from the standalone `tygo generate` command to `make tygo-generate`, matching the "make targets canonical" convention the rest of the file follows, and made the session-end bullet describe the actual scoped `lint-go`/`test-go` and/or `lint-web`/`test-react` invocation rather than implying a single `test-lint` call
- [ ] T055 Run the full-loop check from the end of quickstart.md: agent creates a branch, edits a query, edits a DTO, is blocked on a failing test, commits on the branch, opens a PR, and a planted lint violation shows red before merge — **not run as one continuous scripted sequence**: each component was instead verified individually across the phases above (branch guard in US1; regen-sqlc.sh/regen-tygo.sh finding and fixing real drift, go-postedit.sh reformatting a planted violation, and session-verify.sh blocking-then-unblocking in US4) and opening an actual PR needs a push (see T057). Running them in one uninterrupted script end-to-end is left for whoever pushes this branch
- [X] T056 Confirm the declared gating set in contracts/required-checks.md matches the workflow file exactly — all ten job names (FR-003) — confirmed via `grep -n "^\s*name:" .github/workflows/*.yml`: sqlc generate is up to date, tygo generate is up to date, lint (go), go vet, go test, integration test, frontend test (vitest), frontend typecheck, lint (web), e2e (playwright) — exactly the ten names in contracts/required-checks.md, verbatim
- [ ] T057 Open the pull request for this feature and confirm all ten checks appear and pass — the feature gating itself is the only end-to-end proof — **not run**: this requires pushing this branch to GitHub and opening a PR, which is a user-facing action outside a local implementation pass (and two of the ten checks are already known to fail today regardless — see T039/T041: the pre-existing atsboard integration-test compile break, and pre-existing e2e/UI drift beyond the one fix made in T041). Left for whoever pushes this branch
