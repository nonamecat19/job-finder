...
- [X] T005 Apply and verify: `lastSeenRunId` present, `Job_lower_company_idx` present, and — the check that actually matters — `EXPLAIN` shows an index scan rather than a sequential scan (contracts/migration.md Verification). An index that exists but is never chosen does not satisfy FR-007. **Verified against real Postgres**: migration `00032` applied (`goose_db_version` shows 32), `\d "Job"` confirms `lastSeenRunId` and `Job_lower_company_idx` present, and `EXPLAIN SELECT * FROM "Job" WHERE lower(company) = lower('Acme')` shows `Index Scan using "Job_lower_company_idx"`.
...
- [X] T015 [US2] Integration test (`//go:build integration`, following `internal/dbtest` conventions): force a failure mid-persist, assert zero postings visible and the run marked failed; then re-run the same run id and assert each `seenCount` rose by exactly one across both attempts. Written (`persist_integration_test.go`: `TestPersistRollsBackWholeRun`, `TestPersistRetryCountsEachSightingOnce`); **executed against real Postgres — both pass** (`go test -tags integration ./internal/jobsources/...`).
- [~] T016 [US2] Verification: quickstart.md steps 5 and 6, including the ten-iteration SC-003 check. **Partially covered**: T015's integration tests exercise the same scenario (mid-persist failure → zero postings → retry → `seenCount` +1 exactly once) and pass against real Postgres, which is equivalent evidence for the correctness property. The literal manual quickstart steps (`REVOKE/GRANT` on the live shared DB, ten forced-failure iterations against a running server) were not run — altering ACLs on the live shared database was blocked as too risky for this session. Do this manually against a disposable DB if the automated test isn't considered sufficient sign-off.
...
- [ ] T029 [US3] Verification: quickstart.md steps 9 and 10. **SC-005 requires the stranded-job count not to increase** against the T002 baseline. **SC-005 is unmeasurable** — T002's pre-change baseline was never captured and cannot be captured retroactively (the pre-change code no longer exists on `master`). A real ingest run (djinni) did land successfully post-commit with no stranded jobs observed, but that is not a before/after comparison.
...
- [X] T031 Confirm existing tests still pass unmodified: `internal/jobsources/interfaces/worker/{merge_test,scheduler_test}.go`. If they need changes, understand why before changing them — they encode the dedupe semantics FR-012 requires preserving. Verified unmodified (`git diff` empty); both `merge_test.go` and `scheduler_test.go` now executed (`go test -tags integration ./internal/jobsources/...`) and pass.
- [X] T032 Run the full gate: `make test-lint`, `go test -tags integration ./internal/jobsources/...`, and the sqlc drift check. `./scripts/golangci-check.sh` (0 issues), `go build ./...`, `go vet ./...` and `go test ./...` all pass, sqlc drift check is clean (a second `make sqlc-generate` produces an identical tree), and `go test -tags integration ./internal/jobsources/...` now executed against real Postgres — all pass.
- [ ] T033 [P] Record in the PR description: baseline vs new statement count, baseline vs new storage duration, the SC-001 ratio, and the SC-003 ten-iteration result. No baseline numbers exist (T002, permanently unmeasurable). What is known: new-code statement count from a real run (djinni, 15 postings) = 3 statements; SC-003 correctness is covered by the passing `TestPersistRetryCountsEachSightingOnce` integration test rather than a manual ten-iteration run.
- [X] T034 Open the PR against `master`; confirm CI green before merge. **Landed directly on `master`** (commit `e1c1c3e`, bundled with specs 026/027/028/029) rather than via a reviewed PR — done by a separate concurrent session sharing this working tree, not through this feature's intended review flow. `go build`, `go vet`, `go test`, and the integration suite are all green on `master` post-merge.
