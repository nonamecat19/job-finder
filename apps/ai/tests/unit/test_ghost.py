...
class _FakeBoundModel:
    ...
    supports the one method `ghost.run` calls, `ainvoke`."""
...
def _work(snapshot: dict[str, Any]) -> GhostWork:
    return GhostWork(jobId="job_1", activityId=None, snapshot=snapshot, snapshot_hash="sha256:x")
...
def test_run_returns_parsed_result_when_every_signal_is_known(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake = _FakeChatModel(lambda _msgs: _ai_message(VALID_RESULT))
    monkeypatch.setattr(ghost.gateway, "chat_model", lambda task_key: fake)
...
def test_run_caps_confidence_when_a_signal_is_unknown(monkeypatch: pytest.MonkeyPatch) -> None:
    snapshot = {**FULL_SNAPSHOT, "days_open": None}
    high_confidence = {**VALID_RESULT, "confidence": 0.95}
    fake = _FakeChatModel(lambda _msgs: _ai_message(high_confidence))
    monkeypatch.setattr(ghost.gateway, "chat_model", lambda task_key: fake)

    result, _usage = asyncio.run(ghost.run(_work(snapshot)))

    assert result.confidence == ghost.CONFIDENCE_CAP_WHEN_UNKNOWN
...
def test_run_sends_trace_id_as_gateway_request_metadata(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    asyncio.run(ghost.run(_work(FULL_SNAPSHOT), trace_id="trace_123"))
...
def test_run_retries_once_on_malformed_json_then_succeeds(
    ...
    result, _usage = asyncio.run(ghost.run(_work(FULL_SNAPSHOT)))
...
def test_run_fails_internal_after_exhausting_every_retry(
    ...
    with pytest.raises(CapabilityError) as exc_info:
        asyncio.run(ghost.run(_work(FULL_SNAPSHOT)))

    assert exc_info.value.category == "internal"
    assert exc_info.value.failed_step == "ghost"
    assert fake.bound is not None
    assert len(fake.bound.calls) == ghost.MAX_EXTRA_ATTEMPTS + 1
...
def test_run_rejects_a_malformed_snapshot_as_invalid_input(
    ...
    with pytest.raises(CapabilityError) as exc_info:
        asyncio.run(ghost.run(_work({"title": "only a title"})))
...
def test_capability_declares_one_model_call_bound() -> None:
    assert ghost.CAPABILITY.bounds.max_model_calls == 1
    assert ghost.CAPABILITY.task_key == "ghost"
    assert ghost.CAPABILITY.transport == "event"
    assert ghost.CAPABILITY.layer == "chain"
