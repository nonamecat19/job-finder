...
// The config must survive a restart (FR-002), which an in-memory cache alone
// cannot prove: this exercises persist → construct a fresh service → read
// against a real Postgres, so the migration's seeded row, its CHECK guards and
// the sqlc round-trip are all covered.
func TestResumeShapeSettingPersistsAcrossServiceRestart(t *testing.T) {
	...
	// A fresh database serves the seeded defaults.
	...
	// A new service is what a restarted process sees: the row, not the cache.
...
// The DB-level CHECK constraints are the second line of defence behind the
// service's validation: a write that bypassed Validate must still be rejected.
