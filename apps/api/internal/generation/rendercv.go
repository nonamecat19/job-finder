...
// sectionOrderKey stores the original cv.sections key order captured at
// parse time (see ParseRendercv), so it can survive JSON/YAML round-trips
// through map[string]any (which has no order of its own) as an ordinary
// array value — arrays keep their order through marshalling even though map
// keys don't.
const sectionOrderKey = "_order"

// orderedYAMLMap marshals to a YAML mapping node with keys in a fixed order,
// instead of the alphabetical order gopkg.in/yaml.v3 applies to plain Go
// maps. Used only for cv.sections: rendercv renders sections in the order
// they appear in the YAML, so losing that order (e.g. via generic
// map[string]any marshalling) silently reshuffles the rendered resume.
