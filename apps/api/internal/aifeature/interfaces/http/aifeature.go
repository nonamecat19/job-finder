...
func (h *AiFeatureHandler) update(w http.ResponseWriter, r *http.Request) {
	...
	if !valid {
		httpx.WriteError(w, http.StatusNotFound, "unknown feature: "+feature)
		return
	}
	...
	if body.Threshold < 0 || body.Threshold > 100 {
		httpx.WriteError(w, http.StatusBadRequest, "threshold must be between 0 and 100")
		return
	}
	...
	httpx.WriteJSON(w, http.StatusOK, stateToAiFeatureDto(s))
}
