...
func NewHandler(q Repository, sources *jobsources.Service, djinni adapters.DjinniAdapter, dou adapters.DouAdapter, workua adapters.WorkUaAdapter, indeed adapters.IndeedAdapter, remoteok adapters.RemoteOKAdapter, glassdoor adapters.GlassdoorAdapter, jobleads adapters.JobLeadsAdapter, jobgether adapters.JobgetherAdapter, client Enqueuer, defaultDelay time.Duration, delays map[string]time.Duration) *Handler {
	return &Handler{
		q: q, sources: sources,
		djinni: djinni, dou: dou, workua: workua, indeed: indeed, remoteok: remoteok, glassdoor: glassdoor, jobleads: jobleads, jobgether: jobgether,
		client:       client,
