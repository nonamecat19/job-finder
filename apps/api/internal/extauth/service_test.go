...
func newTestService() (*Service, *fakeRepo) {
	repo := newFakeRepo()
	svc := NewService(repo, NewSigner([]byte("test-secret-32-bytes-of-padding")))
	return svc, repo
}
...
func TestExchangeBootstrapCode_Success(t *testing.T) {
	...
	if pair.Scope != ScopeProfileRead {
		t.Errorf("Scope = %q, want %q", pair.Scope, ScopeProfileRead)
	}
...
func TestExchangeBootstrapCode_ReuseFails(t *testing.T) {
	...
	if _, err := svc.ExchangeBootstrapCode(ctx, code); err != ErrInvalidCode {
		t.Errorf("second exchange (reuse) = %v, want ErrInvalidCode", err)
	}
}

func TestExchangeBootstrapCode_UnknownCode(t *testing.T) {
	svc, _ := newTestService()
	if _, err := svc.ExchangeBootstrapCode(context.Background(), "bogus"); err != ErrInvalidCode {
...
func TestExchangeBootstrapCode_Empty(t *testing.T) {
	svc, _ := newTestService()
	if _, err := svc.ExchangeBootstrapCode(context.Background(), ""); err != ErrInvalidCode {
		t.Errorf("got %v, want ErrInvalidCode", err)
	}
}
...
func TestRefreshTokens_RotatesAndInvalidatesOld(t *testing.T) {
	...
	if _, err := svc.RefreshTokens(ctx, pair.RefreshToken); err != ErrInvalidRefreshToken {
		t.Errorf("reuse of rotated-out token = %v, want ErrInvalidRefreshToken", err)
	}
...
func TestRefreshTokens_UnknownToken(t *testing.T) {
	svc, _ := newTestService()
	if _, err := svc.RefreshTokens(context.Background(), "bogus"); err != ErrInvalidRefreshToken {
